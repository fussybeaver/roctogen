...
pub async fn create_token(claims: JwtClaims, private_key_pem: &str) -> Result<String, JsValue> {
    ...
    let private_key = pem::parse(private_key_pem).map_err(to_js_error)?;
    let crypto_subtle = worker.crypto()?.subtle();

    let key_usages = js_sys::Array::new_with_length(1);
    key_usages.set(0, "sign".into());
    let priv_key_arr = unsafe { Uint8Array::new(&Uint8Array::view(&private_key.contents())) };

    let algo = Algorithm {
        name: "RSASSA-PKCS1-v1_5",
        hash: AlgoHash { name: "SHA-256" },
    };
    ...
    let promise = crypto_subtle.import_key_with_object(
        "pkcs8",
        &priv_key_arr,
        &js_algo,
        false,
    ...
    let mut buffer = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&header);

    buffer.push('.');
    let claims = serde_json::to_string(&claims).map_err(to_js_error)?;
    base64::engine::general_purpose::URL_SAFE_NO_PAD.encode_string(&claims, &mut buffer);

    let promise = crypto_subtle
        .sign_with_object_and_u8_array(&js_algo, &crypto_key, unsafe { buffer.as_bytes_mut() })?;
    let signature_arr_buf = JsFuture::from(promise).await?;

    let sig_uint_arr = js_sys::Uint8Array::new(&signature_arr_buf).to_vec();

    buffer.push('.');
    base64::engine::general_purpose::URL_SAFE_NO_PAD.encode_string(&sig_uint_arr, &mut buffer);
